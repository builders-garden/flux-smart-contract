...
contract OffchainResolver is IExtendedResolver {
    ...
    function init(string memory _url, address[] memory _signers, address _owner) external {
        require(!initialized);
        url = _url;
    ...
    function setSettings(string memory _url, address[] memory _signers) external {
        require(msg.sender == owner, "Only owner can set settings");
        url = _url;
        for(uint i = 0; i < _signers.length; i++) {
            signers[_signers[i]] = true;
        }
